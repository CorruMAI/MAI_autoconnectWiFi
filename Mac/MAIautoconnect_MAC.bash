...
if [[ $# > 1 ]]; then
    ...
    if [[ $command == "--version" || $command == "-v" || $command == "version" || $command == "v" ]]; then
        ...
        echo "    u[ninstall] - uninstalls this script from system"
        echo "    h[elp] - shows this message"
        echo "    v[ersion] - shows current version"
        echo -e "\nThis script use crontab and networksetup. If those utilits aren't installed on your PC script won't work!"
...
while :
do
    SSIDinf=$(networksetup -getairportnetwork en1)
    SSID=`echo ${SSIDinf} | awk '{print $4}'`
